...
Rcpp::List score_infoaug_fun(const arma::mat& cov, const arma::vec& ftime,arma::vec& ltime,
                     const arma::vec& disease, const arma::vec delta,
                     const arma::vec& selvar, const arma::vec& subvar,
                     const arma::vec& kmfit,
                     const arma::vec& beta, 
                     const arma::mat& cond_exp,
                     const Rcpp::StringVector cal_Meat){
  ...
  double alpha = accu(selvar % (1-delta%disease))/accu(1-delta%disease);
  arma::vec rho = delta % disease + (1-delta%disease)%selvar/alpha;
  ...
  for(int i=0;i<etime_len;i++){
    ...
    if(ftime[0] == e_time[i] ){
      ...
      arma::uvec SXi_loc = find(ftime < e_time[i]);
      arma::vec SXi = ftime_0(SXi_loc);
      ...
      arma::vec GXi(SXi.n_elem);
      GXi = kmfit_0.elem(find(GXi_loc1==1 ));
  ...
  S0.each_col() = S0_temp.col(0); /* t*1 */
  
  /* Calculate dLambda_10 */
  ...
  dLambda_10.each_row() = dLambda_sm.row(0); /* n*etime_len  */
  
  
  /*Conditional term for exponential term */

  arma::mat E_temp(n,etime_len);
  E_temp.each_col() = cond_exp.col(p);
  ...
  for(int i=0;i<p;i++){
    ...
    arma::mat ZE_temp(n,etime_len);
    ZE_temp.each_col() = cond_exp.col(i);
    
    arma::mat aug_mt = (ZE_temp - E_expand % E_temp) % risk_matrix % (1-rho_matrix) % Wi_matrix % dLambda_10;
    aug_mt.elem(find_nonfinite(aug_mt)).fill(0);
    
    aug_term(i)=accu(aug_mt);
  ...
  if(cal_Meat(0) == "True"){
    ...
    for(int i=0;i<p;i++){
      ...
      eta4_first.each_col() = (1- delta % disease);
      ...
      arma::mat eta5_mt = (ZE_temp - E_expand % E_temp) % risk_matrix % eta4_first % Wi_matrix % dLambda_10;
      eta5_mt.elem(find_nonfinite(eta5_mt)).fill(0);
      eta5_list.row(i) = (sum(eta5_mt,1)).t();
    ...
    arma::mat eta5_list_sub = eta5_list.each_row() % subvar.t();  
    ...
    arma::mat V1_mt = eta4_list_sub - eta5_list_sub;
    arma::mat V1 =  V1_mt * V1_mt.t() * (1-alpha_star)/alpha_star * 1/alpha_star ;
