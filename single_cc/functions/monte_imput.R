...
monte_imput <- function(ftime,ltime,fstatus,cov,selvar,B=200,
  ...
  alpha <- sum(selvar*( 1-ifelse(fstatus==cencode,0,1)*ifelse(fstatus==failcode,1,0)))/ sum(( 1-ifelse(fstatus==cencode,0,1)*ifelse(fstatus==failcode,1,0)))
  rho <- ifelse(fstatus==cencode,0,1) * ifelse(fstatus==failcode,1,0) + (1-ifelse(fstatus==cencode,0,1)*ifelse(fstatus==failcode,1,0))*selvar/alpha 
